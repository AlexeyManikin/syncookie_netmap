...
inline void initialize_ehhdr(u_int8_t* src_mac, u_int8_t* dst_mac, struct ether_header *eh)
{
...
inline static  uint16_t checksum(const void *data, uint16_t len, uint32_t sum)
{
    ...
    for (i = 0; i < (len & ~1U); i += 2) {
        sum += (u_int16_t)ntohs((uint16_t) *((u_int16_t *)(addr + i)));
        if (sum > 0xFFFF)
            sum -= 0xFFFF;
    }
    /*
...
inline static u_int16_t wrapsum(u_int32_t sum)
{
    sum = ~sum & 0xFFFF;
    return (htons((uint16_t) sum));
}

inline void initialize_iphdr(uint32_t src, uint32_t dst, int l4size, struct iphdr *ip, u_int8_t ipproto)
{
    ip->version = 4;
    ip->ihl = 5;
